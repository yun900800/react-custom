...
16. 过程抽象的定义应该是这样的，过程的使用方式与过程究竟通过哪种更基本的过程实现的具体细节相互分析，就是过程抽象。（简单来说就是过程的使用与过程的实现细节互相分离）；同理数据的使用与数据对象的构造相互分离就是数据抽象。

17. [Concepts that surprised you when you read SICP?](https://stackoverflow.com/questions/355406/concepts-that-surprised-you-when-you-read-sicp)
