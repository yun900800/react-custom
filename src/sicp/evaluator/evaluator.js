...
import { makeName, isName, symbolOfName } from './evaluator-name'
...
import { isBlock, blockBody, listOfUnassigned } from './evaluator-block'
...
const operatorCombinationToApplication = (component) => {
  ...
  return isUnaryOperatorCombination(component)
    ? makeApplication(makeName(operator), list(firstOperand(component)))
    : makeApplication(
        makeName(operator),
        list(firstOperand(component), secondOperand(component))
      )
}
...
const evalSequence = (stmts, env) => {
  if (isEmptySequence(stmts)) {
    ...
    const first_stmt_value = evaluate(firstStatement(stmts), env)
...
const scanOutDeclarations = (component) => {
  return isSequence(component)
    ? accumulate(
        append,
        null,
        map(scanOutDeclarations, sequenceStatements(component))
      )
...
const evalBlock = (component, env) => {
  ...
  return evaluate(body, extendEnviroment(locals, unassigneds, env))
}

const evalReturnStatement = (component, env) => {
  return makeReturnValue(evaluate(returnExpression(component), env))
}
...
const evalAssignment = (component, env) => {
  const value = evaluate(assignmentValueExpression(component), env)
...
const apply = (fun, args) => {
  if (isPrimitiveFunction(fun)) {
    ...
    const result = evaluate(
      functionBody(fun),
      extendEnviroment(functionParameters(fun), args, functionEnvironment(fun))
    )
    return isReturnStatement(result) ? returnExpression(result) : undefined
  } else {
