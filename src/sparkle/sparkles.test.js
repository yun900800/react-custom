...
describe('sparkles test',()=>{
    ...
    it('Sparkles should be render queryByDisplayValue test',()=>{
        const {queryByDisplayValue} = render(<Sparkles color='red'>hello</Sparkles>);
    ...
    it('test Sparkle render',()=>{
        render(<Sparkle size={15} color='red' style={{top:15, left:15}}></Sparkle>);
        //screen.debug();
        expect(screen.getByTestId('sparkle').getAttribute('style')).toEqual('top: 15px; left: 15px;')
        expect(screen.getByTestId('sparkle').children[0].getAttribute('height')).toEqual('15');
        expect(screen.getByTestId('sparkle').children[0].children[0].getAttribute('fill')).toEqual('red');
    });
});